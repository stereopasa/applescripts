    ...
    repeat with i from 1 to number of items in these_items
        ...
        try
            if kind of (info for this_item) is not "folder" then
                set dest_file to destination_directory & "/" & base_name & "_800.mp4"
                do shell script "/usr/local/bin/ffmpeg -i " & quoted form of file_path & " -vf scale=800:-2 " & quoted form of dest_file
            else
